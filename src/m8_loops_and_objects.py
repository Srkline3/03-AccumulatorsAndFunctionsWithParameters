...
def draw_circles1():
    ...
    winder = rg.RoseWindow(400, 400)
    for k in range(21):
        rg.Circle(rg.Point(200, 200), k).attach_to(winder)
    winder.render()
...
def draw_circles2():
    ...
    for k in range(36):
        q = k * 20
        bigbebe = rg.Circle(rg.Point(x + q, y), 10)
        bigbebe.fill_color = 'blue'
        bigbebe.attach_to(window)
    window.render()
...
def draw_circles3():
    ...
    for k in range(100):
        radius = radius + 1
        bigbebe = rg.Circle(rg.Point(x, y), radius)
        bigbebe.attach_to(window)
    window.render()
    window.close_on_mouse_click()
